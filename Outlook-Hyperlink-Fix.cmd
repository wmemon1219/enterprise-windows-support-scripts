...
if %errorlevel% equ 0 set outlook=yes

:AREYOUSURE
echo.
if "%outlook%"=="yes" echo.This will close Outlook to save all your drafts first!
echo.
...
echo.
echo.
CHOICE /N /C:12 /D 2 /T 30 /M "Answer?: "
IF %ERRORLEVEL% EQU 1 GOTO RUNIT
IF %ERRORLEVEL% EQU 2 goto eof

:RUNIT
start /min "" if "%outlook%"=="yes" taskkill /f /im outlook* > NUL
echo Set-ItemProperty -Path Registry::HKEY_CLASSES_ROOT\.html\ -Name '(Default)' -Value htmlfile > %temp%\runit.ps1

if exist %workdir%\windows\scripts\Outlook_Policies_Fix.reg regedit /s %workdir%\windows\scripts\Outlook_Policies_Fix.reg

powershell -f %temp%\runit.ps1
tasklist | findstr /i outlook > Nul
...
if /i not [%logname%]==[NOLOG] set logname=HYPERLINKS
start "" \\toolshed.corp.amazon.com\share\Counts4Daviburt\Scripts\Createlog.cmd
)

if exist "%temp%\runit.ps1" del /q "%temp%\runit.ps1"
:EOFF
exit
